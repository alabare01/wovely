...
import { firstPatternId, patternIdWithCharts, firstCollectionId } from "./lib/supabase-admin.js";
...
async function buildLoggedInTarget(acct) {
  ...
  if (acct.loaded) {
    ...
    target.ids.collection = await firstCollectionId(acct.userId).catch(() => null);
  }
  return target;
}
