  ...
  <div style={{position:"fixed",inset:0,zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:T.sans}}>
    ...
    <div className="fu" style={{position:"relative",zIndex:1,width:"100%",maxWidth:380,background:T.modal,borderRadius:20,padding:"32px 28px",boxShadow:"0 16px 48px rgba(123,106,212,.18)"}}>
      ...
      {!isPro&&<>
        ...
        <div style={{fontSize:11,color:T.ink3,marginBottom:12,lineHeight:1.5,textAlign:"center"}}>Parking saves your progress and frees up your active view.<br/><span onClick={onGoPro} style={{color:T.terra,cursor:"pointer",fontWeight:600}}>Upgrade to Craft for a bigger library →</span></div>
      </>}
...
const ChartsAndImagesSection = ({ pattern, tier, isAnonymous, onShowUpgrade, pinnedImageId, onTogglePin, activeTab, scopedPartName, isMultiPart = false }) => {
  ...
  // Locked (Pro/Free/Anon) path uses a count-only query; unlocked (Craft) path
...
const Detail = ({p,onBack,onSave,pct,estYards,estSkeins,pdfThumbUrl,CSS,Bar,Photo,Stars,WireframeViewer,Btn,scrollToRow:initialScrollToRow,isAnonymous=false,onSignUp,collectionUpgrade,onCollectionUpgrade,onCollectionUpgradeDismiss,tier,onShowUpgrade,pinnedImageId,onTogglePin}) => {
