...
const GuestEmailAsk = () => {
  ...
  const submit=async(e)=>{
    ...
    try{posthog.capture("email_captured",{source:getSource()||"direct",where:"guest_pattern"});}catch{}
    try{pulse("email_captured",{path:window.location.pathname});}catch{}
    setState("saved");
  };
  if(state==="saved")return(
    <div style={{fontFamily:"Nunito,sans-serif",fontSize:14,color:"#1E8A63",fontWeight:700,padding:"10px 0"}}>Saved. Bev will write once, with the code.</div>
  );
  return(
    <form onSubmit={submit} style={{display:"flex",flexDirection:"column",gap:8,maxWidth:360,margin:"0 auto"}}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <input type="email" inputMode="email" autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@somewhere.com" aria-label="Email" disabled={state==="busy"} style={{flex:"1 1 180px",minWidth:0,border:`1.5px solid ${err?"#C2564A":T.border}`,borderRadius:12,padding:"12px 14px",fontSize:15,fontFamily:"Nunito,sans-serif",color:T.ink,background:"#fff",outline:"none"}}/>
        <button type="submit" disabled={state==="busy"} style={{flex:"0 0 auto",background:"#7B6AD4",color:"#fff",border:"none",borderRadius:12,padding:"12px 20px",fontSize:14,fontWeight:700,fontFamily:"Nunito,sans-serif",cursor:state==="busy"?"default":"pointer",boxShadow:"0 4px 16px rgba(123,106,212,0.3)",opacity:state==="busy"?.7:1}}>{state==="busy"?"Saving":"Save my spot"}</button>
      </div>
      <div style={{fontSize:12,color:err?"#C2564A":"#726A92",lineHeight:1.5,fontFamily:"Nunito,sans-serif"}}>{err||"One note from Bev with the code. Nothing else, ever."}</div>
    </form>
  );
};
        ...
        <div style={{position:"relative",marginTop:8}}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              Tick your rows here as you go. Leave an email and Bev sends you the code that halves your first three months of Craft.
            </div>
            <GuestEmailAsk/>
            <div style={{fontSize:12.5,color:"#726A92",marginTop:16}}>
              Want this place on every device?{" "}
              <span
